...
import {IBitape} from "./interfaces/IBitape.sol";
...
contract Main is Ownable {
    ...
    /// @dev Bitape token address.
    address public bitape;

    /// @dev apetoshi fee recipient.
    address public apetoshi;
    ...
    /// @dev Tracks bitape per single hash.
    uint256 public cumulativeBitapePerHash;
    ...
    mapping(address => uint256) public playerBitapeDebt;
    ...
    /// @dev Initial bitape per block
    uint256 public constant INITIAL_BITAPE_PER_BLOCK = 2.5e18; // 2.5 Bitape per block
    ...
    /**
     * @dev Purchase new miners using bitape.
     */
    function buyMiner(uint256 minerIndex, uint256 x, uint256 y) external {
        ...
        if (IBitape(bitape).balanceOf(msg.sender) < miner.cost) {
        ...
        // transfer correct amt of bitape
        IBitape(bitape).transferFrom(msg.sender, address(this), miner.cost);

        // burn
        IBitape(bitape).burn(FixedPointMathLib.mulWad(miner.cost, burnPct));
    ...
    /**
     * @dev Sell miners, if there's a secondary market, player will get bitape in return.
     *
    ...
    function sellMiner(uint256 minerId) external {
        ...
        // if there's not enough bitape in this contract to pay out, revert
        if (secondHandPrice > IBitape(bitape).balanceOf(address(this))) {
        ...
        if (secondHandPrice > 0) {
            IBitape(bitape).transfer(msg.sender, secondHandPrice);
        }
    }
    ...
    function buyNewFacility() external {
        ...
        if (IBitape(bitape).balanceOf(msg.sender) < newFacility.cost) {
            revert Errors.TooPoor();
        }

        IBitape(bitape).transferFrom(msg.sender, address(this), newFacility.cost);

        // burn
        IBitape(bitape).burn(FixedPointMathLib.mulWad(newFacility.cost, burnPct));
    ...
    function claimRewards() external {
        ...
        IBitape(bitape).mint(msg.sender, finalRewards);

        address referrer = referrals[msg.sender];
        if (referrer != address(0)) {
            IBitape(bitape).mint(referrer, referralBonus);
            referralBonusPaid[referrer] += referralBonus;
        } else {
            // else mint referral fee to this contract
            IBitape(bitape).mint(address(this), referralBonus);
    ...
    function _updateCumulativeRewards() internal {
        ...
        // bitape emissions per halvening
        uint256 currentBlock = lastRewardBlock;
        uint256 lastBitapePerBlock =
            INITIAL_BITAPE_PER_BLOCK / (2 ** ((lastRewardBlock - startBlock) / HALVING_INTERVAL));
        ...
        while (currentBlock < block.number) {
            ...
            cumulativeBitapePerHash +=
                ((lastBitapePerBlock * (endBlock - currentBlock) * REWARDS_PRECISION) / totalHashrate);
    ...
    function _updateRewards(address player) internal {
        ...
        playerPendingRewards[player] +=
            (playerHashrate[player] * (cumulativeBitapePerHash - playerBitapeDebt[player])) / REWARDS_PRECISION;

        playerBitapeDebt[player] = cumulativeBitapePerHash;
    }
    ...
    function _increaseHashrate(address player, uint256 hashrate) internal {
        // only start rewarding bitape per block when the first miner is added
        if (!miningHasStarted) {
     ...
     */
    function getBitapePerBlock() public view returns (uint256) {
        if (!miningHasStarted) {
            return 0;
        }
        ...
        return INITIAL_BITAPE_PER_BLOCK / (2 ** halvingsSinceStart);
    }
    ...
    function pendingRewards(address player) external view returns (uint256) {
        ...
        uint256 simulatedCumulativeBitapePerHash = cumulativeBitapePerHash;
        ...
        while (currentBlock < block.number) {
            ...
            if (totalHashrate > 0) {
                simulatedCumulativeBitapePerHash +=
                    ((lastBitapePerBlock * (endBlock - currentBlock) * REWARDS_PRECISION) / totalHashrate);
            }
        ...
        return FixedPointMathLib.mulWad(
            playerPendingRewards[player]
                + (
                    (playerHashrate[player] * (simulatedCumulativeBitapePerHash - playerBitapeDebt[player]))
                        / REWARDS_PRECISION
                ),
            1e18 - referralFee
        );
    ...
    /**
     * @dev Get current bitape a player mines per block.
     */
    function playerBitapePerBlock(address player) external view returns (uint256) {
        if (totalHashrate == 0) {
            return 0; // Prevent division by zero
        }

        uint256 currBitapePerBlock =
            INITIAL_BITAPE_PER_BLOCK / (2 ** ((block.number - startBlock) / HALVING_INTERVAL));

        return FixedPointMathLib.mulDiv(playerHashrate[player], currBitapePerBlock, totalHashrate);
    }
    ...
    function setBitape(address _bitape) external onlyOwner {
        bitape = _bitape;
    }

    function setApetoshi(address _apetoshi) external onlyOwner {
        apetoshi = _apetoshi;
    }
    ...
    function withdraw() external onlyOwner {
        ...
        (bool success,) = payable(apetoshi).call{value: balance}("");

        if (!success) revert Errors.WithdrawFailed();
    }

    function withdrawBitape(uint256 amt) external onlyOwner {
        IBitape(bitape).transfer(apetoshi, amt);
    }
